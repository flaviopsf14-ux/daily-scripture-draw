...
const Index = () => {
  ...
  // Verse view
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-gradient-to-b from-[hsl(255,65%,55%)] via-[hsl(240,60%,50%)] to-[hsl(270,70%,40%)] px-6 py-12 text-white">
      <div className="flex w-full max-w-md flex-col items-center gap-8">
        {/* Back button */}
        <button
          onClick={() => setView("home")}
          className="self-start flex items-center gap-2 text-white/80 transition-colors hover:text-white"
        >
          <ArrowLeft className="h-5 w-5" />
          Voltar
        </button>
        ...
        {/* Verse card */}
        <div
          className={`w-full rounded-3xl bg-white/15 p-8 backdrop-blur-sm transition-all duration-300 ${
        ...
        >
          {currentVerse && (
            <div className="flex flex-col items-center gap-5 text-center">
              ...
              <p className="font-verse text-xl leading-relaxed italic sm:text-2xl">
                "{currentVerse.text}"
              </p>
              ...
              <button
                ...
                className="flex items-center gap-2 rounded-full bg-white/10 px-4 py-2 text-sm transition-all hover:bg-white/20"
              >
                <Heart
                ...
                />
                {isFavorite(currentVerse.id) ? "Favoritado" : "Favoritar"}
              </button>
            </div>
          )}
        </div>
        ...
        <div className="flex w-full flex-col gap-3">
          <button
            onClick={() => drawVerse("all")}
            className="flex w-full items-center justify-center gap-3 rounded-2xl bg-white px-6 py-4 text-base font-semibold text-[hsl(255,65%,55%)] shadow-lg transition-all duration-200 hover:scale-[1.02] hover:shadow-xl active:scale-[0.98]"
          >
            <Sparkles className="h-5 w-5" />
            Sortear Outro
          </button>
          ...
          <div className="flex gap-2">
            <button
              onClick={copyVerse}
              className="flex flex-1 items-center justify-center gap-2 rounded-2xl bg-white/15 px-4 py-3 text-sm font-medium backdrop-blur-sm transition-all hover:bg-white/25"
            >
